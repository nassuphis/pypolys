...
#_mps = ctypes.CDLL(ctypes.util.find_library("mps"))

# Explicit absolute path to libmps.dylib
_mps_path = "/usr/local/lib/libmps.dylib"
if not os.path.exists(_mps_path):
    raise FileNotFoundError(f"Expected MPSolve at {_mps_path}")
...
class MonomialPoly(Polynomial):
    ...
    def set_coefficient(self, n, coeff_re, coeff_im=None):
        ...
        if coeff_im is not None and type(coeff_re) != type(coeff_im):
            raise ValueError("Coefficient's real and imaginary parts have different types")
...
have different types")
        ...
        if isinstance(coeff_re, int):
            ...
            if coeff_im is None:
                coeff_im = 0.0
            _mps.mps_chebyshev_poly_set_coefficient_d(cntxt, cb, n,
                                                      coeff_re, coeff_im)
...
def roots(coeffs: np.ndarray) -> np.ndarray:
    ...
    degree = len(coeffs) - 1  # Degree of the polynomial
    ctx = Context()
    poly = MonomialPoly(ctx, degree)
    for i, coeff in enumerate(coeffs):
        poly.set_coefficient(i, coeff.real, coeff.imag)
