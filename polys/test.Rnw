...
\newpage
\subsection{{\tt mpmath} meets Wilkinson}
...
def show_roots(cf,roots):
    ...
    df = pd.DataFrame({
        ...
        "Residual" : [abs(mp.polyval(cf, r)) for r in roots_sorted]
    })
...
\begin{itemize}
    \item {\tt np.roots} fails miserably on wilkinson.
    \item we use a simple aberth solver that runs a fixed number of iteration to test {\tt mpmath}
    \item {\tt mpmath} fails with 10 digits
    \item {\tt mpmath} eats wilkinson for breakfast at 100 digits
