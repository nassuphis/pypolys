...
\subsection{Taylor Shif}
\subsection{Reversal}

\newpage
\section{Quadratics, Cubics}
...
\subsubsection{{\tt\bf MPSolve} complex number structure}
<<, engine='python',size="small">>=
...
def horner_vec_mpc(coeffs: list[gmpy2.mpc], xs: list[gmpy2.mpc]) -> list[gmpy2.mpc]:
    return [horner_mpc_high_to_low(coeffs, x) for x in xs]
...
\subsubsection{Wilkinson: the Darth Vader of polynomials}
<<, engine='python',size="small">>=
w20 = np.poly1d([1])
